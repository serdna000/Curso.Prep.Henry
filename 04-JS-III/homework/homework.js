...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  let numeroConvertirdo = n.toString();
  let arrayNumero = numeroConvertirdo.split("");
  if (arrayNumero[0] === "9") {
    return true;
  } else {
    return false;
...
function mesesDelAño(array) {
  ...
  for (let i = 0; i < nuevoArray.length; i++) {
    if (nuevoArray[i] === "Enero") {
      contador++;
    } else if (nuevoArray[i] === "Marzo") {
      contador++;
    } else if (nuevoArray[i] === "Noviembre") {
      contador++;
    }
  }
  if (contador === 3) {
